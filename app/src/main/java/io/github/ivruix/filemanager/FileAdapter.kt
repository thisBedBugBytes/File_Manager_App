...
class FileAdapter(private val context: Context, private val files: ArrayList<File>) :
        ...
        init {
            itemView.setOnClickListener {
                ...
                if (position != RecyclerView.NO_POSITION) {
                    onItemClickListener?.onItemClick(files[position])
                }
            }
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        val file = files[position]
        Log.d("FileAdapter", "Binding file: ${file.absolutePath}")
        ...
        try {
                    ...
                    "png", "jpg", "bmp" -> {
                        try {
                            ...
                            if (bitmap != null) {
                                ...
                                Log.w("FileAdapter", "Bitmap decode returned null: ${file.absolutePath}")
                                holder.fileIcon.setImageResource(R.drawable.blank)
                            }
                        } catch (e: Exception) {
                            Log.e("FileAdapter", "Failed to decode image: ${file.absolutePath}", e)
                            holder.fileIcon.setImageResource(R.drawable.blank)
                        }
                    }
            ...
            Log.e("FileAdapter", "Error binding file: ${file.absolutePath}", e)
            holder.fileIcon.setImageResource(R.drawable.blank)
        }
    }
    ...
    private fun getFileTimeOfCreation(file: File): Long {
        return try {
            ...
            Log.w("FileAdapter", "Could not get creation time for ${file.absolutePath}", e)
            0L
        }
    }
